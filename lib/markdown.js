...
function renderMath(source, displayMode = false) {
  return katex.renderToString(source.trim(), {
    ...
    macros: {
      '\\mathbbm': '\\htmlClass{mathbbm}{\\mathrm{#1}}',
    },
  });
}
...
const ANCHOR_PATTERN = /:::anchor\s+([a-z\d][a-z\d-]*)\s+:::/g;

function parseReferenceTarget(raw) {
  const match = raw.trim().match(/^([a-z\d][a-z\d/_-]*)?(?:#([a-z\d][a-z\d-]*))?$/);
  if (!match || (!match[1] && !match[2])) return null;
  return { slug: match[1] || '', anchor: match[2] || '' };
}

function defaultReferenceHref({ slug, anchor }) {
  return `${slug ? `/blog/${slug}/` : ''}${anchor ? `#${anchor}` : ''}`;
}

function renderReference(rawTarget, label, options) {
  const target = parseReferenceTarget(rawTarget);
  if (!target) throw new Error(`The cross-reference target "${rawTarget}" is not a valid post slug or anchor.`);
  const resolved = options.resolveReference
    ? options.resolveReference(target, rawTarget)
    : { href: defaultReferenceHref(target) };
  const preview = resolved.previewId ? ` data-xref-preview="${escapeHtml(resolved.previewId)}"` : '';
  return `<a class="xref" href="${escapeHtml(resolved.href)}"${preview}>${renderInline(label, options)}<span class="xref__mark" aria-hidden="true">§</span></a>`;
}
...
function renderInline(source, options = {}) {
  ...
  while (cursor < source.length) {
    ...
    if (source.startsWith(':::', cursor)) {
      ...
      const anchor = source.slice(cursor).match(/^:::anchor\s+([a-z\d][a-z\d-]*)\s+:::/);
      if (anchor) {
        flushPlain();
        output += `<span class="xref-anchor" id="${escapeHtml(anchor[1])}"></span>`;
        cursor += anchor[0].length;
        continue;
      }

      const reference = source.slice(cursor).match(/^:::ref\s+(\S+)\s+(.+?)\s+:::/);
      if (reference) {
        flushPlain();
        output += renderReference(reference[1], reference[2], options);
