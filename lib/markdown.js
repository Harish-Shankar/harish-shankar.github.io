...
function renderInline(source) {
  ...
  while (cursor < source.length) {
    ...
    if (source[cursor] === '[') {
      ...
      if (labelEnd !== -1 && destinationEnd !== -1) {
        ...
        if (destination && isSafeUrl(destination.url)) {
          ...
          const title = destination.title ? ` title="${escapeHtml(destination.title)}"` : '';
          output += `<a href="${escapeHtml(destination.url)}"${title}>${renderInline(label)}</a>`;
          cursor = destinationEnd + 1;
          continue;
        }
      }
    ...
    for (const [marker, tag] of pairedMarkers) {
      ...
      flushPlain();
      output += `<${tag}>${renderInline(source.slice(cursor + marker.length, end))}</${tag}>`;
...
function parseBlocks(source) {
  ...
  while (cursor < lines.length) {
    ...
    if (/^:::progress\s*$/.test(line.trim())) {
      nodes.push({ type: 'progress' });
      cursor += 1;
      continue;
    }
    ...
    if (marginBlock) {
      ...
      nodes.push({ type: 'marginNote', variant: marginBlock[1], children: parseBlocks(content.join('\n')) });
      cursor += 1;
      continue;
    }
  ...
  return nodes.map((node) => {
    switch (node.type) {
      case 'heading': {
        ...
        return `<h${node.level}${id ? ` id="${escapeHtml(id)}"` : ''}>${renderInline(node.value)}</h${node.level}>`;
      }
      case 'paragraph':
      ...
      case 'codeBlock': {
        ...
        const code = normalizedLanguage === 'python' || normalizedLanguage === 'py'
      ...
      case 'marginNote': {
        const label = node.variant === 'cite' ? 'Citation' : 'Note';
      ...
      case 'progress': {
        const timestamp = new Date(options.buildTimestamp || Date.now());
        if (Number.isNaN(timestamp.getTime())) throw new Error('The build timestamp is invalid.');
      ...
      case 'orderedList': {
        ...
        const items = node.items.map((item) => {
          const task = typeof item.checked === 'boolean'
            ...
            : '';
          return `<li${task ? ' class="task-item"' : ''}>${task}${renderInline(item.value)}</li>`;
        }).join('');
        return `<${tag}>${items}</${tag}>`;
      }
      case 'table': {
        const headers = node.headers.map((cell) => `<th scope="col">${renderInline(cell)}</th>`).join('');
        const rows = node.rows.map((row) => `<tr>${row.map((cell) => `<td>${renderInline(cell)}</td>`).join('')}</tr>`).join('');
