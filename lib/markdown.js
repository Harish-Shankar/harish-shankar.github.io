  ...
  while (cursor < source.length) {
    ...
    if (source.startsWith(':::', cursor)) {
      ...
      if (annotation) {
        ...
        const label = variant === 'cite' ? 'Citation' : variant === 'note' ? 'Note' : 'Definition';
        output += `<span class="margin-note margin-note--inline margin-note--${variant}" role="note" aria-label="${label}"><span class="margin-note-label">${label}</span><span class="margin-note__content">${renderInline(annotation[2], options)}</span></span>`;
